...
- The table is displayed inside `תשלומים אחרים` and has exactly five operational columns: student name; signed debt/credit amount; settlement method; settled/paid state; settlement date.
...
- Cash settlement reverses that sign for accounting cash flow: a student debt creates positive income when paid; a refund to a student creates a negative `oneTimePayments` cash flow in the actual settlement month and therefore reduces that month's income total.
- The deterministic cash row id is scoped by target school year and student so repeated edits update the same settlement instead of duplicating it.
- Choosing `lessons` / card offset keeps a verified prior-year balance as an opening balance in the new-year billing calculation; choosing cash removes it from the new-year annual target so the new year's regular payment grid stays clean.
- The active per-lesson summary is scoped to the selected Sep–Aug school year. Historical per-lesson lessons/payments no longer contaminate the new-year running total; a verified prior-year balance enters only when explicitly carried as `lessons` / offset.
- Legacy per-lesson years do not persist the historical lesson price for every old lesson. The system must therefore show `נדרש אימות` and require an explicit amount instead of recomputing a potentially false balance from today's rate.
